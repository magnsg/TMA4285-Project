...
```{r}
data <- readMat("~/Documents/Tidsrekker/Prosjekt/Mouse28-140313_BS0150_HMMready.mat")
...
for (i in 1:length(idx)) {
  states = c(states, rep(i, idx[i]))
}
plot_data = data.frame(ang = ang, x=1:length(ang), states=as.factor(states))
```

```{r}
ggplot(data = plot_data) + geom_point(aes(x = x, y=ang, col = states))
```
